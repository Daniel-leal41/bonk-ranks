 ...
 *   F → Marrom
 *
 * Exposto como window.RankColors (IIFE), na convenção dos demais módulos
 * de features/shared/ — nada vaza para o escopo global.
 */
...
(function () {
  ...
  function getRankGradientCSS(rank) {
    var colors = getRankConfig(rank).colors;
    return 'linear-gradient(135deg, ' + colors.join(', ') + ')';
  }

  function applyRankColor(element, rank) {
    if (!element) return;
    var glow = getRankConfig(rank).glow;
  ...
  window.RankColors = {
    getConfig: getRankConfig,
    getGradientCSS: getRankGradientCSS,
    applyRankColor: applyRankColor
  };
})();
